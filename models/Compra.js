  ...
  store: async (datos, usuario_id, empresa_id, externalConn = null) => {
    ...
    try {
      ...
      for (const item of tmpItems) {
        ...
        // Actualizamos stock con la cantidad real (unidades o gramos)
        await conn.execute(
          "UPDATE productos SET stock = stock + ? WHERE id = ?",
      ...
      // ... (El resto del método: Pagos, Arqueos y Deuda se mantiene igual)
      // Asegurate de copiar el bloque de pagos original aquí abajo
      return true;
    } catch (error) {
      throw error;
    }

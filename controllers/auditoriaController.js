...
//     // 1. RÁFAGAS DE DEVOLUCIONES (Requiere 3 o más en 1 hora)
//     const [devoluciones] = await db.execute(
//       `
//       SELECT d1.usuario_id, u.name as usuario_nombre, d1.caja_id,
//         COUNT(*) as cantidad_devoluciones,
//         MIN(d1.created_at) as inicio_periodo, MAX(d1.created_at) as fin_periodo
//       FROM devoluciones d1
//       JOIN users u ON d1.usuario_id = u.id
//       WHERE d1.empresa_id = ? AND d1.created_at >= DATE_SUB(NOW(), INTERVAL 48 HOUR)
//       GROUP BY d1.usuario_id, d1.caja_id, DATE_FORMAT(d1.created_at, '%Y-%m-%d %H')
//       HAVING cantidad_devoluciones >= 3
//     `,
//       [empresa_id]
...
//     // 2. 🚀 FALTANTES DE ARQUEO RECALCULADOS (Lógica BI Pura) 🚀
//     // No confiamos en la columna 'diferencia', la calculamos nosotros
//     const [faltantes] = await db.execute(
//       `
//       SELECT
//         a.id as arqueo_id, u.name as usuario_nombre, a.caja_id, a.fecha_cierre,
//         (
//           a.monto_final - (
//             a.monto_inicial +
//             IFNULL((SELECT SUM(efectivo) FROM ventas WHERE arqueo_id = a.id), 0) +
//             IFNULL((SELECT SUM(monto) FROM movimiento_cajas WHERE arqueo_id = a.id AND tipo = 'Ingreso' AND descripcion NOT LIKE 'Venta%'), 0) -
//             IFNULL((SELECT SUM(monto) FROM movimiento_cajas WHERE arqueo_id = a.id AND tipo = 'Egreso'), 0) -
//             IFNULL((SELECT SUM(monto) FROM retiros_caja WHERE arqueo_id = a.id), 0)
//           )
//         ) as diferencia_real
//       FROM arqueos a
//       JOIN users u ON a.usuario_id = u.id
//       WHERE a.empresa_id = ? AND a.estado = 'Cerrado'
//       HAVING diferencia_real < -50 -- Solo faltantes mayores a $50
//       ORDER BY a.fecha_cierre DESC LIMIT 10
//     `,
//       [empresa_id]
//     );
...
//     res.json({
//       devoluciones_sospechosas: devoluciones || [],
//       arqueos_con_faltante:
//         faltantes.map((f) => ({ ...f, diferencia: f.diferencia_real })) || [],
//       horarios_extranos: horarios || [],
//       ranking_generosidad: descuentos || [],
//       tickets_anulados: anulaciones || [],
//     });
...
const getAnomalias = async (req, res) => {
  try {
    ...
    // 1. RÁFAGAS DE DEVOLUCIONES (3 o más en 1 hora)
    const [devoluciones] = await db.execute(
      `
      SELECT d1.usuario_id, u.name as usuario_nombre, d1.caja_id,
        COUNT(*) as cantidad_devoluciones,
    ...
    // 2. 🚀 FALTANTES DE ARQUEO (Lógica Sincronizada 100%) 🚀
    const [faltantes] = await db.execute(
      `
      SELECT 
        a.id as arqueo_id, u.name as usuario_nombre, a.caja_id, a.fecha_cierre,
        (
          a.monto_final - (
            a.monto_inicial + 
            IFNULL((SELECT SUM(monto) FROM movimiento_cajas WHERE arqueo_id = a.id AND tipo = 'Ingreso'), 0) - 
            IFNULL((SELECT SUM(monto) FROM movimiento_cajas WHERE arqueo_id = a.id AND tipo = 'Egreso'), 0) - 
            IFNULL((SELECT SUM(monto) FROM retiros_caja WHERE arqueo_id = a.id), 0)
          )
      ...
      JOIN users u ON a.usuario_id = u.id
      WHERE a.empresa_id = ? AND a.estado = 'Cerrado'
      HAVING diferencia_calculada < -50 -- Solo faltantes reales mayores a $50
      ORDER BY a.fecha_cierre DESC LIMIT 10
    `,
      [empresa_id]
    );
    ...
    res.json({
      devoluciones_sospechosas: devoluciones || [],
      arqueos_con_faltante:
        faltantes.map((f) => ({ ...f, diferencia: f.diferencia_calculada })) ||
        [],
      horarios_extranos: horarios || [],
      ranking_generosidad: descuentos || [],
      tickets_anulados: anulaciones || [],

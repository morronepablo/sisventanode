...
const getPrediccionCompra = async (req, res) => {
  try {
    ...
    const query = `
      ...
      LEFT JOIN unidads u ON p.unidad_id = u.id
      WHERE p.empresa_id = ?
      -- 🟢 Cambiamos el orden para que por defecto los críticos (stock <= mínimo) vengan primero
      ORDER BY (p.stock <= p.stock_minimo) DESC, ventas_30_dias DESC
    `;
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        -- 3. Última Salida Real (GREATEST de 2 subconsultas independientes para evitar error de Scope)
        GREATEST(
    ...
    // Pasamos los parámetros: today, emp, today, emp, emp, emp, emp
    const [rows] = await db.execute(query, [
      today,
      empresa_id,
      today,
    ...
    const reporte = rows.map((r) => {
      ...
      let diffDays = 999;
      if (ultima) {
        // Formateamos para comparar solo fechas sin horas que ensucien el cálculo
        const ultimaNormalizada = new Date(
          new Date(ultima).toISOString().split("T")[0] + "T00:00:00",
        );
        diffDays = Math.floor(
